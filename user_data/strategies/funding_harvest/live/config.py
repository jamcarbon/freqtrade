...
from funding_harvest.research.backtest import DATA as _DATA  # repo-absolute data dir
...
class StrategyConfig:
    # --- data / universe (repo-absolute, cwd-independent) --------------------
    data_dir: str = _DATA
    borrow_file: str = os.path.join(_DATA, "borrow_rates.json")
    oi_file: str = os.path.join(_DATA, "oi_snapshot.json")
    ...
    maker_timeout_s: int = 20   # how long a postOnly leg may rest before re-pricing
    poll_interval_s: int = 2    # order-status poll cadence
    max_reprice: int = 3        # times to chase the touch before giving up the leg
